"""
Google Gemini 图片编辑适配器
支持通过 Gemini API 进行图像生成/编辑
"""
...
class GeminiAdapter:
    ...
    def process_image(self,
        ...
        try:
            ...
            # 3. 调用 Gemini API
            if progress_callback:
                progress_callback("调用 Gemini API...", 30)

            result_image = self._call_gemini_api(image_base64, mime_type, prompt, ref_data_list)

            if not result_image:
                return ProcessingResult(success=False, message="Gemini API 处理失败")
    ...
    def _call_gemini_api(self, image_base64: str, mime_type: str, prompt: str, ref_data_list: list = None) -> Optional[Image.Image]:
        """
        ...
        try:
            # 增加超时时间到 300 秒（5分钟），大图片或复杂处理需要更长时间
...
def process_with_gemini(image_path: str,
                       api_key: str,
